...
public class Moviment : MonoBehaviour {
	...
	private void moviment()
	{
		...
		direx = Input.GetAxis ("mvVert");
		direy = Input.GetAxis ("mvUp");

        if (direx == -1) { transform.eulerAngles = new Vector3(0, 270, 0);
        rb.velocity = new Vector3(speed*-1, 0,0 );
       
        }
        if (direx == 1) { transform.eulerAngles = new Vector3(0, 90, 0);
